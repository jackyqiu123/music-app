...
function Triangle({synth, setSynth}:InstrumentProps):JSX.Element{
    ...
    return(
        <div className = "Triangle">
             <img  src={triangle} alt="triangle" useMap = "#triangle" width = "1190" height = "340"/>
             <map id = "triangle" name = "triangle">            
                {keys.map(key=>{
                    return(
                        <area key = {key.coords} title = {key.note} shape = "circle" coords = {key.coords} href = "" onClick = {handleClick}/>
                    )
                })}
             </map>
        </div>
